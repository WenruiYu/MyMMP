...
def main_try_test_local_audio():
    ...
    video_content = "Hello, I am Puluulu. How are you? 你好，我是噗嚕嚕。"
...
def main_generate_ai_video(video_generator):
    ...
    # Clear mix video flag for regular videos
    st.session_state['is_mix_video'] = False
    ...
    with video_generator:
        ...
        with st_area as status:
            ...
            st.write(f"✓ {tr('Generate Video subtitles...')} ({subtitle_time:.1f}s)")
            ...
            st.write(f"✓ {tr('Get Video Resource...')} ({resource_time:.1f}s)")
            ...
            if enable_subtitles:
                ...
                add_subtitle_time = time.time() - start_time
                st.write(f"✓ {tr('Add Subtitles...')} ({add_subtitle_time:.1f}s)")
            
            # Calculate total time
            total_time = dubbing_time + subtitle_time + resource_time + normalize_time + video_gen_time + add_subtitle_time
    ...
    with video_generator:
        ...
        with st_area as status:
            ...
            for video_dir, audio_file in zip(video_dir_list, audio_file_list):
                ...
                st.write(f"  • {tr('Mix Video Scene')} {i+1}: {scene_time:.1f}s")
                i = i + 1
            ...
            # Calculate total time
            total_time = dubbing_time + normalize_time + subtitle_time + video_gen_time + add_subtitle_time
