...
from tools.font_utils import get_font_list_for_ui
...
default_fonts_dir = os.path.join(script_dir, "../fonts")
default_fonts_dir = os.path.abspath(default_fonts_dir)
...
with bg_music_container:
    ...
    with llm_columns[1]:
        ...
        with nest_columns[0]:
            col1_sub = st.container()
            with col1_sub:
                st.checkbox(label=tr("Enable background music"), key="enable_background_music", value=True)
                st.checkbox(label=tr("Random music selection"), key="random_background_music", value=False,
                           disabled=not st.session_state.get("enable_background_music", True))
        with nest_columns[1]:
            # Only show dropdown if not using random selection
            if not st.session_state.get("random_background_music", False):
                ...
                st.info(tr("Random selection enabled"))
                # Clear the specific selection when random is enabled
