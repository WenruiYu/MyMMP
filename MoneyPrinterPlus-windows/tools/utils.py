...
def preprocess_tts_text(text):
    ...
    Args:
        text: The input TTS text
        
    Returns:
        Processed text with punctuation marks replaced by newlines
    """
    if not text:
        return text
...
def get_file_from_dir(file_dir, extension):
    extension_list = [ext.strip().lower() for ext in extension.split(',')]
    # 确保提供的是绝对路径
...
def get_random_music_from_dir(music_dir):
    """Randomly select a music file from the directory"""
    import random
    if not music_dir or not os.path.exists(music_dir):
        print(f"WARNING: Music directory does not exist or is empty: {music_dir}")
        return None
    
    # Get all music files
    music_files = get_file_from_dir(music_dir, ".mp3,.wav,.m4a,.flac,.ogg")
    
    if not music_files:
        print(f"WARNING: No music files found in directory: {music_dir}")
        return None
    
    selected_music = random.choice(music_files)
    print(f"INFO: Randomly selected music file: {os.path.basename(selected_music)}")
