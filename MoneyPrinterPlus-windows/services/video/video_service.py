...
class VideoMixService:
    def __init__(self):
        from tools.utils import get_random_music_from_dir
        ...
        random_selection = st.session_state.get("random_background_music", False)
        background_music_dir = st.session_state.get("background_music_dir", None)
        
        if random_selection and background_music_dir:
            # Use random selection
            self.background_music = get_random_music_from_dir(background_music_dir)
        else:
            # Use manually selected music file
            self.background_music = st.session_state.get("background_music", None)
            # If no specific file selected and directory exists, still try random
            if not self.background_music and background_music_dir:
                print("INFO: No specific music file selected, using random selection as fallback")
                self.background_music = get_random_music_from_dir(background_music_dir)
...
class VideoService:
    def __init__(self, video_list, audio_file):
        ...
        background_music_input = st.session_state.get("background_music", None)
        background_music_dir = st.session_state.get("background_music_dir", None)
        
        # If no specific music file selected, try to get random from directory
        if not background_music_input and background_music_dir:
            self.background_music = get_random_music_from_dir(background_music_dir)
        else:
            self.background_music = background_music_input
